...
describe('ai-risk-mapping', () => {
  it('maps AI security levels to risk levels', () => {
    ...
    expect(toRiskLevel('위험', 3, null)).toBe('high');
    expect(toRiskLevel('주의', 9, null)).toBe('medium');
    expect(toRiskLevel('주의', 5, null)).toBe('medium');
    expect(toRiskLevel('양호', 2, null)).toBe('low');
    expect(toRiskLevel('양호', 1, null)).toBe('low');
  });
  ...
  it('falls back to score when security level is unknown', () => {
    ...
    expect(toRiskLevel('UNKNOWN', 5, null)).toBe('high');
    expect(toRiskLevel('UNKNOWN', 3, null)).toBe('medium');
