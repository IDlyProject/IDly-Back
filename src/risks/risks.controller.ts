...
class UpdateActionStatusDto {
  @ApiProperty({ enum: ['resolved', 'skipped', 'pending'], example: 'resolved' })
  @IsEnum(['resolved', 'skipped', 'pending'])
  status: 'resolved' | 'skipped' | 'pending';

  @ApiProperty({ example: ['action-item-id-1', 'action-item-id-2'], required: false })
  @IsArray()
  @IsString({ each: true })
  @IsOptional()
...
- \`completedStepIds\`: 완료한 체크리스트 항목 ID 배열 (선택)`,
  ...
  @ApiResponse({ status: 201, description: '조치 상태 저장됨', schema: { example: { serviceAccountId: 'sa-uuid', status: 'resolved', resolvedAt: '2026-07-17T00:00:00.000Z', homeDelta: { actionRequiredCount: 2, securityScore: 78 } } } })
  @ApiResponse({ status: 404, description: '서비스를 찾을 수 없음' })
...
복원 기능은 현재 미구현(추후 지원 예정).`,
  ...
  @ApiParam({ name: 'serviceAccountId' })
  @ApiResponse({ status: 201, description: '휴면 전환 완료', schema: { example: { serviceAccountId: 'sa-uuid', status: 'dormant' } } })
  @ApiResponse({ status: 404, description: '서비스를 찾을 수 없음' })
